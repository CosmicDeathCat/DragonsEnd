...
public class EquipmentItem : Item, IEquipmentItem
{
    ...
    public virtual void ApplyModifiers(IActor target, bool isEquipping)
    {
        if (this is IWeaponItem weaponItem)
        {
            ...
            if (isEquipping) target.DamageMultiplier.AddModifier(modifierValue);
        ...
        if (this is IArmorItem armorItem)
        {
            ...
            if (isEquipping) target.DamageReductionMultiplier.AddModifier(modifierValue);
        ...
        if (isEquipping) target.ActorStats.AddModifier(Stats);
